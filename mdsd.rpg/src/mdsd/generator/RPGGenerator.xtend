...
class RPGGenerator extends AbstractGenerator {
	...
	def CharSequence generateGame(String classFileName){
		...
		public class «classFileName»{
			...
			private void executeMove(Move move, String moveName, Entity target, Entity user, HashMap<String, Number> eff){
				if(!move.getMove(moveName).getMoveEffects().isEmpty()){
					for(EffectMove moveEffect: move.getMove(moveName).getMoveEffects()){
						moveEffect.doEffect(move, moveName, target, user, eff);
					}
				}
			}
			...
			private void executeAfter(Move move, String moveName, Entity target){
		   	...
		   	private void fight(Entity enemyEntity){
		   		...
		   		while(fighting){
					...
					HashMap<String, Number> mapEff = new HashMap<>();
					try{
					...
					while (pickMove){  
						...
						if(moves.contains(moveName)){
							...
							executeMove(move, moveName, enemyEntity, playerEntity, mapEff);
					...
					if(deathChecker.check(enemyEntity)){
						...
						executeMove(move, enemyEntity.getMoveNameList().get(choosenMove), playerEntity, enemyEntity, mapEff);
	...
	def CharSequence generateEffectMove(){
    	...
    	public abstract class EffectMove {
    	    ...
    	    public abstract void doEffect(Move move, String name, Entity enemy, Entity player, HashMap<String, Number> effMap);    	
    	}
    	'''
		}
	...
	def CharSequence generateMoveEffectFile(MoveE moveE, Attributes attributes){
    	...
    	'''
			...
			public class «moveE.name» extends EffectMove{
				...
				@Override		
				public void doEffect(Move move, String name, Entity target, Entity user, HashMap<String, Number> eff){
					if(effectMove(move, name, user)){
						Number modifier = calculateModifier(move, name, target, user, eff);
				...
				public Number calculateModifier(Move move, String name, Entity target, Entity user, HashMap<String, Number> eff) {
					Number modifier = 1.0;
					...
					try {
						String locationType = "";
						Number effectiveness = 1.0;
						for (Map.Entry<String, Number> entry : eff.entrySet()) {
							if(entry != null) {
								locationType = entry.getKey();
								effectiveness = entry.getValue();
							}
						}
						if(moveType == locationType) {
							modifier = modifier.floatValue() * effectiveness.floatValue();
						}
					} catch(Exception e) {
						// Do nothing
					}
