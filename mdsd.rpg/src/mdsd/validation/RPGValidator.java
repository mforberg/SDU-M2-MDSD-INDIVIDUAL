...
//	}
	...
	public void checkDuplicateRelations(Relations relations) {
		...
		if(!dupes.isEmpty()) {
			warning("There are duplicated type declartions: " + dupes, (EStructuralFeature) RPGPackage.Literals.RELATIONS__TYPE);
		}
		
	}
	
	@Check
	public void checkEntityMoveAttributes(SystemRPG sysrpg) {
		HashSet<String> observed_attributes = new HashSet<String>();
		
		for(Declaration dec : sysrpg.getDeclarations()) {
			if(dec instanceof Effects) {
				for(Effect e : ((Effects) dec).getEffect()) {
					if(e instanceof MoveE) {
						String t = e.getRule().getTarget().getName();
						observed_attributes.add(t);
						for(Rule me : ((MoveE) e).getExtraRule()) {
							observed_attributes.add(me.getTarget().getName());
						}
					}			
				}
			}
		...
		for(Declaration dec : sysrpg.getDeclarations()) {
			if(dec instanceof Entities) {
				for(Entity e : ((Entities) dec).getEntity()) {
					HashSet<String> entityAttributes = new HashSet<>();
					
					for(AltAttribute a : e.getAtt()) {
						entityAttributes.add(a.getAttribute().getName());
					}
					
					if(e.getExtend() != null) {
						for(Entity other_e : ((Entities) dec).getEntity()) {
							if(other_e.getName() == e.getExtend().getName()) {
								for(AltAttribute a : other_e.getAtt()) {
									entityAttributes.add(a.getAttribute().getName());
								}
							}
						}
					...
					for(String s : observed_attributes) {
						if(!entityAttributes.contains(s)) {
							error("Risking null pointer: " + e.getName() + " is missing: " + s, RPGPackage.Literals.SYSTEM_RPG__NAME);
						}
					}
